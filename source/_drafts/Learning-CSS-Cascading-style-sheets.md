...
Now lets look at the font-style property. There are 4 possible values inherit, normal, italic, oblique. The inherit is a valid value for many properties it means use the same value that the parent element has. italic means use the italic version of the font if you have one, normal means use the normal version of the font and oblique says use the oblique version of the font if you have one. Oblique is different from italic and oblique means that you are requesting the browser to skew the font by a certain amount which is very helpful when there is no italic version of the font. italic means I prefer to use the italic version or oblique version of the font instead of skewing the type.

Now lets take a look at the font-weight property. The possible values for this property are either keywords or numbers. The valid keywords are bold, bolder, inherit, light, normal, lighter and the valid number are 100, 200, 300, ... 900 with 100 being the lightest and 900 being the boldest. usually numbers from 100, 200, 300 and 400 represent lighter version of the font, 500, 600 represent the normal version of the font and 700, 800, 900 represent the bolder version of the font. h2 element by default is bold so if you do `h2 { font-weight: normal; }` it will make the h2 heading text not to be bold.

The font that you are dealing with has a lot to do with the capabilities for what the font-weight and font-style property can do. You can get over what you do with them if you know the properties of font-family that you are working with. There is also a CSS property to capitalize or represent the characters in lower case using the text-transform property. The accepted values for the property are inherit, capitalize, lowercase and uppercase. capitalize will capitalize the first letter of every word, lowercase will represent all the characters n lowercase letters, uppercase will represent all the letter in uppercase.
